...
import { buffers, channel, END, eventChannel } from '../src'
...
assert.ok = function ok(actual) {
  return expect(actual).toBeTruthy()
}

test('Unbuffered channel', done => {
  ...
  try {
    ...
    assert.ok(
      /provided with an undefined/.test(e.message),
